...
// the MCP server, and exercises the tools over real MCP Streamable HTTP —
// exactly the way Claude Code talks to the server. The bridge now has exactly
// one mode: http bound to loopback.
//
...
const PORT = 8794; // bridge server (http mode); distinct from the 8787 default
...
// The bridge now has exactly one mode: http bound to loopback. It is started
// as a child here rather than spawned per MCP session the way stdio used to.
...
// and the browser profile in `finally`, rather than leaking a process that
// holds port 8794 for the next run.
let context = null;
...
try {
  ...
  // Every client.callTool("<name>", args) call below predates the http port
  // and is unchanged from the stdio-mode client — this thin wrapper is the
  // only thing that adapts to the SDK client's own callTool({name, arguments}).
  const client = {
    callTool: async (name, args = {}) => await mcpClient.callTool({ name, arguments: args }),
  };

  const serverInfo = mcpClient.getServerVersion();
  check("MCP initialize", serverInfo?.name === "claude-chrome", JSON.stringify(serverInfo));
  ...
  // Launch Chromium with the extension, then point it at the test bridge via
  // storage — the extension reads wsUrl from chrome.storage.local; seed it by
  // evaluating in the service worker context through Playwright.
  userDataDir = mkdtempSync(join(tmpdir(), "cc-bridge-e2e-"));
  ...
  // Point the extension at the test bridge and reconnect.
  ...
  if (!connected) {
    // Thrown, not process.exit(1): the latter would skip the `finally` below
    // and leave the server child and the browser profile behind.
    throw new Error("Extension never connected; aborting.");
  }
  ...
  // through. Hits /ws directly (not the bare origin) since the http-mode server
  // routes by path.
